import { classify, NO_CLASIFICADO } from '@jogi/classifier';
import { extract } from '@jogi/extract';
import { extractCedulaFace, splitCompositeCedula, isUnreadable } from '@jogi/cedula';
import path from 'path';
import fs from 'fs';
import { createRequire } from 'module';
import { doctypesCatalog, getDoctypesMap } from '@jogi/doctypes';
...
function classificationCacheKey(fileHash2, model, promptVersion) {
  return createHash("sha256").update(fileHash2 + model + promptVersion).digest("hex").slice(0, 32);
}
...
var DOCTYPES_CONTENT_HASH = createHash("sha256").update(JSON.stringify(doctypesCatalog)).digest("hex").slice(0, DOCTYPES_HASH_HEX_LENGTH);
...
function candidateDoctypesHash(candidateDoctypes) {
  return createHash("sha256").update(JSON.stringify(candidateDoctypes)).digest("hex").slice(0, 8);
}
